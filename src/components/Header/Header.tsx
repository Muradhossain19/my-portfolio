...
const Header = () => {
  ...
  // Scroll event listener for box shadow
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <>
      <header
        className={`bg-[#ECF0F3] sticky top-0 z-50 border-b-[1px] border-gray-200 h-[80px] cursor-pointer transition-all duration-300 ${
          isScrolled
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            ...
            {/* Mobile Menu Items with Improved Wave Animation */}
              ...
              {navItems.map((item) => (
                <li key={item.name}>
                  {item.isPage ? (
                    <Link
                      href="/blog"
                      className="text-sm font-normal text-[#3c3e41] hover:text-[#FF004F] transition-colors duration-200"
                      onClick={() => setIsMenuOpen(false)}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => handleSectionScroll(item.section)}
                      className="text-sm font-normal text-[#3c3e41] hover:text-[#FF004F] transition-colors duration-200 bg-transparent text-left w-full"
                    >
                      {item.name.toUpperCase()}
                    </button>
                  )}
